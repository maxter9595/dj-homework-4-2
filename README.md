# Алгоритм запуска проекта и добавления тегов
...
3. Создайте БД с именем, указанным в NAME (netology_m2m_relations):
```bash
...
6. Заведите суперпользователя (необходимо для добавления тегов):
```bash
...
8. Далее зайдите в админку (http://127.0.0.1:8000/admin), введите логин и пароль, добавьте пару тегов (```Теги > ДОБАВИТЬ ТЕГ +```), добавьте теги к статьям (```Статьи > выбор текущей статьи или ДОБАВИТЬ СТАТЬЮ +```)


# Текст основного задания ("Связь многие-ко-многим")
